 ...
 * description: For every dependency your lockfile pins — npm, pnpm and yarn, PyPI, crates.io and Go — compare the version you actually have against the public registry and report what a version number hides — the publishing account changed, the license changed, an install hook appeared, or a known vulnerability already sits on the exact version you pinned. Read-only, no credentials, no adapters, and nothing the repository ships is ever executed. A package name stays the same through a maintainer handover, a sold project, or a takeover, so the name is not the supply-chain trust anchor; the publishing account and the license are, and neither appears in `npm outdated` or a lockfile diff. Reads package-lock.json, npm-shrinkwrap.json, pnpm-lock.yaml (v5 to v9) and yarn.lock (classic and berry), for Python projects uv.lock, poetry.lock, Pipfile.lock or a ==-pinned requirements.txt, for Rust projects Cargo.lock, and for Go projects go.mod — direct dependencies by default, scope=all for the whole tree — asks registry.npmjs.org, pypi.org, crates.io, or proxy.golang.org plus deps.dev for the locked version and for `latest`, and reports each package as CURRENT, BEHIND, FLAGGED (PUBLISHER_CHANGED, LICENSE_CHANGED, MAINTAINERS_REPLACED, LATEST_DEPRECATED, INSTALL_SCRIPT_ADDED when the newer version gains a preinstall/install/postinstall hook, PROVENANCE_DROPPED when the version you have carries a Sigstore build attestation and the newer one does not, SIZE_JUMP when the unpacked tarball at least triples, LOCKED_YANKED when the PyPI or crates.io release you pinned was withdrawn, REQUIRES_PYTHON_CHANGED, MSRV_CHANGED) or UNCHECKED when the registry did not answer — a fetch failure is never reported as "same". Every checked version is also queried against osv.dev (npm, PyPI, crates.io or Go ecosystem) in one batch call, so a KNOWN_VULNERABILITY against the exact version you have is reported first, and an OSV outage is printed as not checked rather than as clean. It says what it cannot know — a publisher change is the account that ran the publish, so a handover to a CI token looks identical to a takeover and is a signal to look at, not a verdict; it reads metadata, never tarballs, so behaviour changes are out of scope; a bun lockfile is named as unsupported rather than silently skipped. Its only network access is anonymous GETs to registry.npmjs.org, pypi.org, crates.io, proxy.golang.org or api.deps.dev and one anonymous POST to api.osv.dev. A local path is inspected in place; a URL is shallow-cloned to a temp directory.
 ...
 *   type: string
 *   required: true
 *   description: 'Project to check: a git URL (https://, ssh://, git@host:owner/name) or a path to a local checkout with an npm, pnpm, yarn, uv, poetry, pip, Cargo or Go module file'
